...
from printing_detection_precrop_nozzle import PrintingDetector
...
class RobotClient:
    ...
    def read_text(self, var: str) -> str:
        try:
            return _to_text(self._osv.read(var))
        except Exception:
            return ""

    def write_var(self, var: str, value) -> bool:
        """Best-effort write using OpenShowVar.

        IMPORTANT:
        Some `py_openshowvar` builds require the value to be a *string* (and may
        optionally take a third `debug` argument). This method tries multiple
        signatures and only returns False after all attempts fail.
        """
        if self._osv is None or value is None:
            return False

        w = getattr(self._osv, "write", None)
        if w is None:
            return False

        # KUKA variables are typically written as text
        v_str = str(value)
        ...
        for args in attempts:
            ...
            try:
                ...
                # Don't bail early; keep trying other signatures
                pass

            # Try with a debug flag if supported: write(var, value, debug=False)
            try:
                if len(args) == 1:
                    w(args[0], False)
                elif len(args) == 2:
                    w(args[0], args[1], False)
                else:
                    w(*args, False)
                return True
            except TypeError:
    ...
    def set_ov_pro(self, ov_percent: float) -> bool:
        try:
            ov = float(ov_percent)
        except Exception:
            return False
        ov = max(0.0, min(100.0, ov))
        # $OV_PRO is usually an INT percent, but some setups accept float
        ok = self.write_var("$OV_PRO", int(round(ov)))
        if not ok:
            ok = self.write_var("$OV_PRO", ov)
        return ok
...
class CaptureWorker(QtCore.QThread):
    ...
    def __init__(
        ...
        # -----------------------------
        # Auto safety override (optional)
        # If NOT PRINTING for >= timeout, we can set $OV_PRO to a target value (e.g., 0).
        # This is OFF by default and must be enabled explicitly in the UI.
        # -----------------------------
        self.no_print_ov_enabled = False
        self.no_print_ov_timeout_s = 10.0
        self.no_print_ov_target = 0.0
        self.no_print_ov_restore = False

        self._no_print_since: Optional[float] = None
        self._ov_prev: Optional[float] = None
        self._ov_applied: bool = False
        self._ov_last_action_t: float = 0.0
    ...
    def set_no_print_override(self, enabled: bool, timeout_s: float, target_ov: float, restore_on_print: bool):
        self.no_print_ov_enabled = bool(enabled)
        self.no_print_ov_timeout_s = max(0.1, float(timeout_s))
        self.no_print_ov_target = float(target_ov)
        self.no_print_ov_restore = bool(restore_on_print)

        # reset state when toggling
        self._no_print_since = None
        self._ov_prev = None
        self._ov_applied = False
        self._ov_last_action_t = 0.0

    @QtCore.pyqtSlot(int)
    def ui_set_ov_pro(self, ov_percent: int):
        """Set $OV_PRO on the worker thread."""
        if self.robot is None:
            self.sig_status.emit("OV write: robot not connected")
            return
        try:
            ov = max(0, min(100, int(ov_percent)))
            ok = self.robot.set_ov_pro(float(ov))
            if ok:
                s = self.robot.read_text("$OV_PRO")
                self.sig_ov.emit(s)
                self.sig_status.emit(f"OV write OK: $OV_PRO={ov}")
            else:
                self.sig_status.emit("OV write FAILED (write rejected or variable not writable)")
        except Exception as e:
            self.sig_status.emit(f"OV write error: {e}")

    def _tick_no_print_override(self, now: float, det_state: Optional[dict]):
        """Called in the capture loop. Uses last detection state to decide if we should apply $OV_PRO."""
        if det_state is not None:
            ...
            if printing:
                # if we were in overridden state, optionally restore previous OV when printing resumes
                if self._ov_applied and self.no_print_ov_restore and self._ov_prev is not None and self.robot is not None:
                    if self.robot.set_ov_pro(self._ov_prev):
                        self.sig_ov.emit(str(self._ov_prev))
                        self.sig_status.emit(f"OV restore: $OV_PRO={self._ov_prev}")
                    else:
                        self.sig_status.emit("OV restore failed (write rejected or variable not writable)")
                self._no_print_since = None
                self._ov_applied = False
            else:
                if self._no_print_since is None:
                    self._no_print_since = float(now)

        # If not enabled, do nothing
        if not self.no_print_ov_enabled:
            return
        if self.robot is None:
            return

        # If we haven't seen a NOT PRINTING state yet, nothing to do
        if self._no_print_since is None:
            return

        # Trigger condition
        if (float(now) - float(self._no_print_since)) < float(self.no_print_ov_timeout_s):
            return

        # Avoid spamming writes
        if self._ov_applied:
            return
        if (float(now) - float(self._ov_last_action_t)) < 1.0:
            return

        # Store previous OV once (best effort) so we can restore if requested
        if self.no_print_ov_restore and self._ov_prev is None:
            try:
                ov_s = self.robot.read_text("$OV_PRO")
                ov = safe_float(ov_s)
                if ov is not None:
                    self._ov_prev = float(ov)
            except Exception:
                pass

        target = max(0.0, min(100.0, float(self.no_print_ov_target)))
        ok = self.robot.set_ov_pro(target)
        self._ov_last_action_t = float(now)

        if ok:
            self._ov_applied = True
            self.sig_ov.emit(str(int(round(target))))
            self.sig_status.emit(f"OV applied: NOT PRINTING for {self.no_print_ov_timeout_s:.1f}s -> $OV_PRO={target:.0f}")
        else:
            self.sig_status.emit("OV apply failed (write rejected or variable not writable)")
    ...
    def run(self):
        ...
        while not self._stop:
            ...
            try:
                ...
                if rgb is not None:
                    ...
                    if self.det_enabled and self._detector is not None:
                        ...
                        if (self._det_frame_i % self.det_every_n) == 0:
                            full_nozzle = None
                            try:
                                full_nozzle = self._detector.detect_best_nozzle(full_bgr)
                            except Exception:
                                full_nozzle = None
                            ...
                            external_nozzle = None
                            try:
                                if full_nozzle is not None:
                                    nconf, fnx, fny, fbox = full_nozzle
                                    if (cx <= int(fnx) < (cx + cw)) and (cy <= int(fny) < (cy + ch)):
                                        x1, y1, x2, y2 = [int(v) for v in fbox]
                                        external_nozzle = (
                                            float(nconf),
                                            int(fnx) - int(cx),
                                            int(fny) - int(cy),
                                            (int(x1) - int(cx), int(y1) - int(cy), int(x2) - int(cx), int(y2) - int(cy)),
                                        )
                            except Exception:
                                external_nozzle = None

                            try:
                                det_vis_bgr, st_det, dbg = self._detector.update(crop_bgr_in, external_nozzle=external_nozzle)
                                det_state = {
                                    ...
                                    "nozzle_source": dbg.get("nozzle_source", ""),
                ...
                # Optional safety override (OFF by default)
                try:
                    self._tick_no_print_override(t_wall, det_state)
                except Exception:
                    pass
...
class VisualWindow(QtWidgets.QWidget, _ImageLabelMixin):
    ...
    def __init__(self):
        ...
        # legend
        try:
            from printing_detection_precrop_nozzle import class_color
    ...
    def on_frame(self, payload: dict):
        ...
        if det_state is not None:
            ...
            nozzle_source = str(det_state.get("nozzle_source", ""))
            src_txt = f", nozzle={nozzle_source}" if nozzle_source else ""
            self.det_reason.setText(
                f"Reason: overlap_tip={overlap_tip:.3f}, new_area_roi={new_area*100:.2f}%, on={on_count}, off={off_count}{src_txt}"
            )
...
class SetupWindow(QtWidgets.QWidget):
    ...
    def __init__(self, worker: CaptureWorker):
        ...
        # auto OV override (optional safety)
        self.ov_enable = QtWidgets.QCheckBox("Auto set $OV_PRO when NOT PRINTING for N seconds")
        self.ov_enable.setChecked(False)
        self.ov_enable.stateChanged.connect(self.on_override_changed)

        self.ov_timeout = QtWidgets.QDoubleSpinBox()
        self.ov_timeout.setRange(0.1, 300.0)
        self.ov_timeout.setValue(10.0)
        self.ov_timeout.setSingleStep(1.0)
        self.ov_timeout.setSuffix(" s")
        self.ov_timeout.setDecimals(1)
        self.ov_timeout.valueChanged.connect(self.on_override_changed)

        self.ov_value = QtWidgets.QSpinBox()
        self.ov_value.setRange(0, 100)
        self.ov_value.setValue(0)
        self.ov_value.setSuffix(" %")
        self.ov_value.valueChanged.connect(self.on_override_changed)

        self.ov_restore = QtWidgets.QCheckBox("Restore previous $OV_PRO when printing resumes")
        self.ov_restore.setChecked(False)
        self.ov_restore.stateChanged.connect(self.on_override_changed)

        self.ov_manual_value = QtWidgets.QSpinBox()
        self.ov_manual_value.setRange(0, 100)
        self.ov_manual_value.setValue(100)
        self.ov_manual_value.setSuffix(" %")

        self.ov_apply_btn = QtWidgets.QPushButton("Apply OV now")
        self.ov_apply_btn.clicked.connect(self.apply_manual_ov)

        self.ov_readback_lbl = QtWidgets.QLabel("Current $OV_PRO: -")
        ...
        form.addRow("Safety override:", QtWidgets.QLabel(""))
        form.addRow(self.ov_enable)
        form.addRow("No-print timeout:", self.ov_timeout)
        form.addRow("Set $OV_PRO to:", self.ov_value)
        form.addRow(self.ov_restore)
        form.addRow("Manual $OV_PRO:", self.ov_manual_value)
        form.addRow("", self.ov_apply_btn)
        form.addRow("", self.ov_readback_lbl)
    ...
    def on_override_changed(self):
        enabled = bool(self.ov_enable.isChecked())
        timeout_s = float(self.ov_timeout.value())
        target = float(self.ov_value.value())
        restore = bool(self.ov_restore.isChecked())

        # IMPORTANT: This can affect robot motion. Keep it explicit and off by default.
        self.worker.set_no_print_override(enabled, timeout_s, target, restore)
        if enabled:
            self.on_status(f"OV override armed: if NOT PRINTING for {timeout_s:.1f}s -> $OV_PRO={target:.0f}")
        else:
            self.on_status("OV override disabled")

    def apply_manual_ov(self):
        ov = int(self.ov_manual_value.value())
        ...
        if not ok:
            self.on_status("OV write dispatch failed")

    @QtCore.pyqtSlot(str)
    def on_ov_readback(self, value: str):
        txt = str(value).strip() if value is not None else "-"
        self.ov_readback_lbl.setText(f"Current $OV_PRO: {txt}")
...
class AppController(QtCore.QObject):
    def __init__(self):
        ...
        self.worker.sig_ov.connect(self.setup.on_ov_readback)
