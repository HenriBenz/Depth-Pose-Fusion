...
try:
    from py_openshowvar import openshowvar  # type: ignore
    OPNSHOWVAR_OK = True
    OPNSHOWVAR_ERR = None
except Exception as e:
    openshowvar = None  # type: ignore
    OPNSHOWVAR_OK = False
    OPNSHOWVAR_ERR = e
...
class RobotClient:
    def __init__(self, ip: str, port: int = 7000):
        if openshowvar is None:
            raise ImportError(f"py_openshowvar not available: {OPNSHOWVAR_ERR}")
        self.ip = ip
        self.port = port
...
class MainWindow(QtWidgets.QWidget):
    def __init__(self):
        ...
        for lab in [self.rgb_full_label, self.depth_label, self.rgb_crop_label, self.det_label]:
            lab.setMinimumSize(420, 240)
            lab.setAlignment(QtCore.Qt.AlignCenter)
            lab.setStyleSheet("background:#111; border:1px solid #333;")
        ...
        layout.addWidget(self.rgb_full_label, 0, 0)
        layout.addWidget(self.depth_label, 0, 1)
        layout.addWidget(self.rgb_crop_label, 1, 0)
        layout.addWidget(self.det_label, 1, 1)
        layout.addWidget(self.plot, 0, 2, 2, 1)
        layout.addWidget(ctrl, 2, 0, 1, 3)
    ...
    def on_detection_changed(self):
        ...
        if enabled:
            if not TORCH_OK:
                ...
                try:
                    detector = PrintingDetector(model)
